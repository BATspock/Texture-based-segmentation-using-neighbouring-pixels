...
class Textons(object):
    def __init__(self, image, cluster_centers, iterations, kernel_size = 2,type_of_assignment=0):
        """
        inputs:
            1.image ==> numpy array (grayscaled image)
            2.number of cluster centers ==> integer
            3.number of iterations ==> integers
            4.kernel size ==> Number of neighbouring pixel (will be stored in a [2*k X 2*k] mask)
    ...
    def textons(self):
        ...
        features_for_kmeans, color_image = I.add_neighbour_info(2)
